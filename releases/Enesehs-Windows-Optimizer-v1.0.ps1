...
function Show-MainMenu {
    while ($true) {
        ...
        $menuLines = @(
            ...
            "║  [4] Windows Tweaks                                                                          ║",
            "║  [5] Tweak Apps                                                                              ║",
            "║  [6] Windows License Activation [BETA]                                                       ║",
            "║  [7] About                                                                                   ║",
            "║  [8] Exit                                                                                    ║",
            "╚══════════════════════════════════════════════════════════════════════════════════════════════╝"
        )
...
function Invoke-SystemFileRepair {
    ...
    try {
        ...
        Write-Host "  Running SFC /scannow..." -ForegroundColor DarkGray
        ...
        Write-Host "  Running DISM ScanHealth..." -ForegroundColor DarkGray
        Start-Process DISM -ArgumentList "/Online /Cleanup-Image /ScanHealth" -Wait -NoNewWindow
        
        Write-Host "  Running DISM RestoreHealth..." -ForegroundColor DarkGray
        Start-Process DISM -ArgumentList "/Online /Cleanup-Image /RestoreHealth" -Wait -NoNewWindow
        
        Write-Host "  Running DISM StartComponentCleanup..." -ForegroundColor DarkGray
...
function Invoke-FileCleanup {
    ...
    if (-not (Confirm-Action "`nProceed with cleanup?")) {
    ...
    foreach ($path in $PathsToClean) {
        if (Test-Path $path) {
            try {
                ...
                Write-Host "  ⚠ Partial clean: $path" -ForegroundColor Yellow
            }
        }
    }
    
    Write-Host "`nCleaning driver residues..." -ForegroundColor Yellow
    ...
    Write-Host "`nRecreating essential folders..." -ForegroundColor Yellow
    foreach ($path in $PathsToClean) {
    ...
    Write-Host "`n✓ File cleanup completed!" -ForegroundColor Green
...
"@
        ...
        $newContent = $hostsContent.TrimEnd() + $newBlock
